...
def recibo_cestaticket_html(id):
    try:
        ...
        html = f'''... [Aqui va tu HTML del recibo] ...'''
...
def recibo_nomina_html(id_nomina):
    try:
        ...
        # [Desempaquetado y formateo HTML igual que antes]
        return html, 200, {'Content-Type': 'text/html'}
    except Exception as e:
...
def generar_recibo_cestaticket_matriz(id):
    try:
        ...
        cur.close(); conn.close()
        if not row: return jsonify({'error': 'Cestaticket no encontrado'}), 404
        # [El resto del código de matriz sigue igual que el anterior]
...
def reporte_pasivos():
    ...
    # [Resto del cálculo de pasivos sin cambios]
...
# ============================================
# 🚀 OPTIMIZACIÓN ESPECIAL: HISTORIAL DE NÓMINAS (YA NO DARÁ TIMEOUT)
# ============================================
...
def get_lotes():
    ...
    # 🛡️ Seguridad: Forzamos un timeout de 60 segundos para que la BD no se cuelgue si tarda.
    cur.execute("SET statement_timeout = '60s'")
    
    # 1️⃣ PRIMER PASO: Traemos los lotes (CONSULTA INSTANTÁNEA)
    query = "SELECT id_lote, descripcion, fecha_calculo, total_usd, total_bs, cantidad_empleados FROM lotes_nomina WHERE 1=1"
    params = []
    if search:
        query += " AND (descripcion ILIKE %s OR CAST(id_lote AS TEXT) ILIKE %s)"
        sp = f"%{search}%"
        params.extend([sp, sp])
    query += " ORDER BY fecha_calculo DESC, id_lote DESC"
    ...
    try:
        ...
        # 2️⃣ SEGUNDO PASO: Extraemos los IDs de los lotes para la segunda consulta.
        lote_ids = [r[0] for r in rows]
        sucursales_map = {}
        
        if lote_ids:
            # Esta consulta es ultrarrápida porque usa un IN con IDs específicos.
            placeholders = ','.join(['%s'] * len(lote_ids))
            cur.execute(f"""
                SELECT n.lote_id, STRING_AGG(DISTINCT s.nombre, ', ')
                FROM nominas n
                JOIN empleados e ON n.id_empleado = e.id_empleado
                JOIN sucursales s ON e.sucursal_id = s.id_sucursal
                WHERE n.lote_id IN ({placeholders})
                GROUP BY n.lote_id
            """, lote_ids)
            
            for row_suc in cur.fetchall():
                sucursales_map[row_suc[0]] = row_suc[1]
    ...
    # 3️⃣ TERCER PASO: Mapeamos las sucursales a sus lotes correspondientes
    return jsonify([{
        'id_lote': r[0],
        'descripcion': r[1],
        ...
        'sucursales_involucradas': sucursales_map.get(r[0], 'Sin sucursal')
    } for r in rows])
...
def manejar_lote(id):
    if request.method == 'GET':
        # [Código GET sin cambios, es rápido porque solo busca 1 lote]
        pass
    elif request.method == 'DELETE':
